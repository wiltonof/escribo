...
public class EnumIntType implements UserType, ParameterizedType {
	...
	private Method recreateIntMthd;
	...
	public void setParameterValues(Properties parameters) {
		if (parameters != null) {
			...
			String strMthd = parameters.getProperty("recreateIntMthd");
			String className = parameters.getProperty("enumClassName");
			Class<?> returnType = null;

			try {
				enumClass = Class.forName(className);
				recreateIntMthd = enumClass.getMethod(strMthd, new Class[] {});
				returnType = recreateIntMthd.getReturnType();
	...
	public Object nullSafeGet(ResultSet rs, String[] names, SessionImplementor session, Object owner) throws HibernateException, SQLException {
		...
		if (names != null && names.length > 0 && rs.getString(names[0]) != null) {
			value = Integer.parseInt(rs.getString(names[0]));
		}
	...
	public void nullSafeSet(PreparedStatement st, Object value, int index, SessionImplementor session) throws HibernateException, SQLException {
		...
		if (value == null) {
			...
			try {
				prepStmtVal = (Integer) recreateIntMthd.invoke(value, new Object[] {});
	...
	public Object deepCopy(Object value) throws HibernateException {
		if (value == null) {
			...
			try {
				Integer valueEnum = (Integer) value.getClass().getMethod("getCod").invoke(value);
				value = enumClass.getMethod("recreateEnum", int.class).invoke(enumClass, valueEnum);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
